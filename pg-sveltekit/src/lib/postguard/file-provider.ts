import { CRYPTIFY_URL, FILEREAD_CHUNK_SIZE } from '$lib/config';
	...
	return new ReadableStream(
		{
			async pull(cntrl) {
				...
				const read = await file.slice(offset, offset + FILEREAD_CHUNK_SIZE).arrayBuffer();
...
async function initFile(
	...
	const response = await fetch(`${CRYPTIFY_URL}/fileupload/init`, {
		signal: abortSignal,
		method: 'POST',
	...
	if (response.status !== 200) {
		...
		throw new Error(
			`Error initializing file upload. status: ${response.status}, msg: ${errorText}`
		);
	}
...
async function storeChunk(
	...
	const response = await fetch(`${CRYPTIFY_URL}/fileupload/${state.uuid}`, {
		signal: abortSignal,
		method: 'PUT',
		headers: {
	...
	if (response.status !== 200) {
		const errorText = await response.text();
		throw new Error(`Error uploading chunk. status: ${response.status}, msg: ${errorText}`);
	}
...
async function finalize(abortSignal: AbortSignal, state: FileState, size: number): Promise<void> {
	const response = await fetch(`${CRYPTIFY_URL}/fileupload/finalize/${state.uuid}`, {
		signal: abortSignal,
		method: 'POST',
		headers: {
			cryptifytoken: state.token,
	...
	if (response.status !== 200) {
		const errorText = await response.text();
		throw new Error(`Error finalizing upload. status: ${response.status}, body: ${errorText}`);
	}
}
...
export async function getFileLoadStream(
	...
	if (response.status !== 200) {
		const errorText = await response.text();
		throw new Error(`Error fetching file. status: ${response.status}, body: ${errorText}`);
	}

	const filesize = parseInt(response.headers.get('content-length') as string);
	const stream = response.body;
	if (stream === null) throw new Error('No response.body object.');
...
export function getFileStoreStream(
	...
	const start = async (c: WritableStreamDefaultController) => {
		try {
			state = await initFile(abortController.signal, recipient, mailContent);
			progressReported(processed, false);
			if (abortController.signal.aborted) throw new Error('Abort signaled during initFile.');
		} catch (e) {
			c.error(e);
		}
	};
	...
	const write = async (chunk: Uint8Array, c: WritableStreamDefaultController) => {
		try {
			...
			progressReported(processed, false);
			if (abortController.signal.aborted) throw new Error('Abort signaled during storeChunk.');
		} catch (e) {
			c.error(e);
		}
	};
	...
	const close = async () => {
		...
		if (abortController.signal.aborted) throw new Error('Abort signaled during finalize.');
	};
	...
	return new WritableStream({ start, write, close, abort }, queuingStrategy);
}
