...
- `pg-manual`'s `build` script carries `--fail-on-warnings`. Webpack reports a broken *static* import as a warning and still exits 0, which is how both breakages recorded under "pg-manual build notes" below could have shipped green; the flag turns them into a failed build. The tree compiles with zero warnings today, so keep it there rather than dropping the flag.
